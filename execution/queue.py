...
(`task.queued` / `task.claimed` / `task.completed` / `task.failed`). Ownership is
a durable event, so a task can never be silently lost: it is QUEUED, CLAIMED
(by worker_id), DONE, or FAILED — never "in some worker's memory".
...
class TaskQueue:
    ...
    def claim(self, worker_id: str) -> Task | None:
        ...
        row = self._conn.execute(
            ...
            (TaskStatus.CLAIMED.value, worker_id, TaskStatus.QUEUED.value),
        ).fetchone()
        self._conn.commit()
        if row is None:
            return None
        task = Task(task_id=row[0], title=row[1], status=TaskStatus.CLAIMED)
        self._emit(EventType.TASK_CLAIMED, task, {"worker_id": worker_id})
        return task
